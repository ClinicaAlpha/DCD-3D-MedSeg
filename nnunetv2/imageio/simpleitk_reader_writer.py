    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        ...
        if 'sitk_stuff' not in properties:
            if 'nibabel_stuff' in properties:
                print("WARNING: SimpleITKIO received properties without 'sitk_stuff'. Falling back to NibabelIO for export.")
                ...
                if 'reoriented_affine' in properties['nibabel_stuff']:
                    NibabelIOWithReorient().write_seg(seg, output_fname, properties)
                else:
                    NibabelIO().write_seg(seg, output_fname, properties)
                return
            raise KeyError("SimpleITKIO.write_seg expected key 'sitk_stuff' in properties")
...
class SimpleITKIOWithReorient(SimpleITKIO):
    ...
    def write_seg(self, seg, output_fname, properties):
        ...
        if 'sitk_stuff' not in properties:
            if 'nibabel_stuff' in properties:
                print("WARNING: SimpleITKIOWithReorient received properties without 'sitk_stuff'. Falling back to NibabelIO for export.")
                from nnunetv2.imageio.nibabel_reader_writer import NibabelIO, NibabelIOWithReorient
                if 'reoriented_affine' in properties['nibabel_stuff']:
                    NibabelIOWithReorient().write_seg(seg, output_fname, properties)
                else:
                    NibabelIO().write_seg(seg, output_fname, properties)
                return
            raise KeyError("SimpleITKIOWithReorient.write_seg expected key 'sitk_stuff' in properties")
        output_dimension = len(properties['sitk_stuff']['spacing'])
        assert 1 < output_dimension < 4
        if output_dimension == 2:
            seg = seg[0]
