...
class DistillationTrainer(nnUNetTrainer):
    ...
    Example usage:
        ...
        >>> # Or load one of the released YAML configs
        >>> config = DistillationConfig.from_yaml('configs/dcd_brats2024_gli.yaml')
        >>>
        >>> # Create trainer
    ...
    def _apply_config_overrides(self):
        ...
        if cfg.num_iterations_per_epoch is not None:
            self.num_iterations_per_epoch = cfg.num_iterations_per_epoch
        ...
        if cfg.batch_size is not None:
            # This will be applied in configure_rotation_dummyDA_mirroring_and_inital_patch_size
            self._override_batch_size = cfg.batch_size
    ...
    def build_network_architecture(self,
        ...
        if cfg.student_architecture is not None:
            ...
            self.print_to_log_file(f"  Using custom student architecture: {modified_arch}")
    ...
    def initialize(self):
        ...
        if cfg.strategy != 'none' and cfg.teacher_checkpoint is not None:
            ...
            self.print_to_log_file("\n[WARN] No distillation (baseline training)")
        ...
        if cfg.strategy != 'none':
            self.print_to_log_file("\n[INFO] Registering feature extraction hooks...")
        ...
        # Print summary
        self.print_to_log_file("\n" + "=" * 60)
        self.print_to_log_file("[OK] Initialization complete!")
        self.print_to_log_file("=" * 60)
    ...
    def _load_teacher_model(self) -> nn.Module:
        ...
        if missing_keys or unexpected_keys:
            self.print_to_log_file("   [WARN] Teacher checkpoint loaded with key mismatches")
            if missing_keys:
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        if self.grad_scaler is not None:
            ...
            grad_norm_tensor = torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
    ...
    def visualize_step(self, batch: dict) -> dict:
        """
        Forward-only step that triggers distillation visualization without any optimization.
        """
        iter_start = time.time()
        data = batch['data']
        target = batch['target']
        ...
        self.iteration_in_epoch += 1
        kd_weight = self.kd_weight_scheduler(self.current_epoch) if self.kd_weight_scheduler is not None else 0.0
        loss_distill_value = loss_distill.item() if isinstance(loss_distill, torch.Tensor) else float(loss_distill)
        batch_time = time.time() - iter_start

        if self.iteration_in_epoch % self._iter_log_interval == 0 or self.iteration_in_epoch == 1:
            self.print_to_log_file(
                f"- nnunetv2 - VisualizeOnly "
                f"[{self.current_epoch}][{self.iteration_in_epoch}]  "
                f"time: {batch_time:.3f}  "
                f"loss_kd(raw): {loss_distill_value:.3f}  kd_weight(schedule): {kd_weight:.3f}"
            )
    ...
    def on_validation_epoch_end(self, val_outputs: list):
        ...
        if self.distill_config.early_stopping_patience > 0:
            ...
            if epochs_no_improve >= self.distill_config.early_stopping_patience:
                self.print_to_log_file(f"\n{'='*60}")
                self.print_to_log_file("[WARN] Early stopping triggered!")
    ...
    def run_training(self):
        ...
        """
        if self.distill_config.visualize_only:
    ...
    def run_visualization_only(self):
        """
        Run a lightweight forward-only loop to produce visualization outputs.
        No optimizer steps and no checkpoint saving.
        """
        num_batches = max(1, int(self.distill_config.visualize_only_batches))
        self.print_to_log_file("=" * 60)
        self.print_to_log_file("Visualize-only mode enabled")
        self.print_to_log_file(f"  Batches: {num_batches}")
        self.print_to_log_file("=" * 60)
        ...
        for _ in range(num_batches):
            vis_outputs.append(self.visualize_step(next(self.dataloader_train)))

        self.print_to_log_file(
            f"Visualize-only run complete. Processed {len(vis_outputs)} batches."
        )
        ...
        empty_cache(self.device)
        self.print_to_log_file("Visualize-only done.")
